...
class LibretroFrontend: ObservableObject {
    ...
    private var retro_get_system_av_info: (@convention(c) (UnsafeMutablePointer<retro_system_av_info>) -> Void)?
    ...
    private func contextReset() {
        
    }
    
    // MARK: - Callbacks
    
    private let videoRefreshCallback: @convention(c) (UnsafeRawPointer?, Int32, Int32, Int) -> Void = { (data, width, height, pitch) in
    ...
    private let inputStateCallback: @convention(c) (Int32, Int32, Int32, Int32) -> Int16 = { (port, device, index, id) in
        return globalLibretroFrontend?.handleInputState(port: port, device: device, index: index, id: id) ?? 0
    }
    
    private func handleVideoRefresh(_ data: UnsafeRawPointer?, width: Int32, height: Int32, pitch: Int) {
    ...
    private func handleInputState(port: Int32, device: Int32, index: Int32, id: Int32) -> Int16 {
        return inputState[port]?[id] == true ? 1 : 0
                ...
                // MARK: - Gamepad Handling
                    ...
                    private func updateAnalogState(_ stick: GCControllerDirectionPad, port: Int32, axisX: Int32, axisY: Int32) {
                    ...
                    private func copyPPSSPPResources(completion: @escaping () -> Void) {
                        DispatchQueue.global(qos: .userInitiated).async {
                            ...
                            guard let ppssppSourceURL = Bundle.main.url(forResource: "PPSSPP", withExtension: nil) else {
